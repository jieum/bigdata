...
public class ZNodeTest {
	static Logger logger = Logger.getLogger(ZNodeTest.class);

	public static void main(String[] args) throws Exception {
		int sessionTimeout = 10 * 1000;
		// 1. ZooKeeper 서버(클러스터)에 연결
		ZooKeeper zk = new ZooKeeper("127.0.0.1:2182", sessionTimeout, null);
		...
		logger.debug("getData [/test01]: " + new String(test01Data));
		...
		logger.debug("getData after setData [/test01/sub01]: " + new String(subData));

		// 6. 노드가 존재하는지 확인
		logger.debug("exists[/test01/sub01]: " + (zk.exists("/test01/sub01", false) != null));
		logger.debug("exists[/test01/sub03]: " + (zk.exists("/test01/sub03", false) != null));

		// 7. /test01의 자식 노드 목록 가져오기
		List<String> children = zk.getChildren("/test01", false);
		for (String eachChildren : children) {
			logger.debug("getChildren [/test01]: " + eachChildren);
		}
