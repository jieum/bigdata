...
import kr.co.jaso.blog.cassandra.generated.BlogReply;
...
public class BlogClient {
  public static void main(String[] args) throws Exception {
    ...
    article.setContents(ByteBuffer.wrap("Hello thrift example is very easy very easy".getBytes()));
    article.addToReplies(reply1);
    article.addToReplies(reply2);
    ...
    System.out.println("===========searchBlogByUserId==========");
    ...
    for(BlogArticle eachArticle: blogArticles) {
      ...
      for(BlogReply eachReply: eachArticle.getReplies()) {
        byte[] replyBytes = new byte[eachReply.getReplyContent().position()];
        eachReply.getReplyContent().get(replyBytes);
        System.out.println("\t" + eachReply.getUserName() + ": " + new String(replyBytes));
      }
    }
    System.out.println("=======================================");

    blogArticles = client.searchBlogByKeyword("user01", "easy");
    System.out.println("===========searchBlogByKeyword==========");
    if(blogArticles.isEmpty()) {
      System.out.println("No Article");
    }
    for(BlogArticle eachArticle: blogArticles) {
